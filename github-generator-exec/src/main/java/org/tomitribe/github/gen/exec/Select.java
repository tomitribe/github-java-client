...
public class Select {
    ...
    public static void main(final String[] args) throws IOException {
        ...
        final URL resource = Select.class.getClassLoader().getResource("openapi.json");
        final String json = IO.slurp(resource);
        final OpenApi openApi = OpenApi.parse(json);

        final File testClasses = JarLocation.jarLocation(Select.class);
        final File target = testClasses.getParentFile();

        final OpenApi issues = Normalizer.builder()
                .operationId(Filter.builder()
                        .include("issue")
                        .include("projects-v2")
                        .build())
                .build().apply(openApi);
        ...
        final File issuesJson = new File(module, "issues.json");

        final Jsonb jsonb = JsonbInstances.get();
        jsonb.toJson(issues, new FileWriter(issuesJson));
//
